...
8. [telafs: file system bridge](#8-telafs-file-system-bridge)
9. [Complete standalone walkthrough](#9-complete-standalone-walkthrough)
10. [Transport layers](#10-transport-layers)
11. [Security model](#11-security-model)
12. [Using Tela with Awan Saya](#12-using-tela-with-awan-saya)
...
| `telafs` | File system bridge | Any machine with an active `tela connect` session |

All four are single Go binaries. No runtime dependencies.
...
```bash
# Linux / macOS example
chmod +x tela telad telahubd telafs
sudo mv tela telad telahubd telafs /usr/local/bin/
```
...
Tela negotiates the best available transport for each connection. See [section 10](#10-transport-layers) for details.
...
## 8. telafs: file system bridge

### What telafs does

`telafs` is a WebDAV server that exposes Tela file shares as a mounted drive. It connects to a running `tela` session through the control API and presents each connected machine's file share as a top-level folder. Windows Explorer, macOS Finder, and Linux file managers can browse, copy, rename, move, and delete remote files as if they were local.

`telafs` requires no kernel drivers, no FUSE, and no third-party software. It uses the standard WebDAV protocol that all major operating systems support natively.

### Starting telafs

First, establish a `tela connect` session to one or more machines that have file sharing enabled. Then start telafs:

```bash
telafs                    # default port 18080
telafs -port 9999         # custom port
```
...
In Finder: Go -> Connect to Server -> `http://localhost:18080/`
...
# Command line (davfs2)
sudo mount -t davfs http://localhost:18080/ /mnt/tela
```

### Drive layout

The root of the drive lists each connected machine that has file sharing enabled. Subdirectories are the remote file system within the machine's shared directory.
...
telafs caches directory listings for 3 seconds to reduce network round trips during browsing. The cache is automatically invalidated when you create, delete, rename, or move files through the same telafs instance. Changes made by other users or processes on the remote machine will appear within 3 seconds.

### Environment variables

| Variable | Default | Description |
|----------|---------|-------------|
| `TELAFS_PORT` | `18080` | WebDAV listen port |

### Running telafs as a service

`telafs` has built-in service management for all platforms:
...
telafs service status
telafs service uninstall
```
...
| Platform | Path |
|----------|------|
| Linux / macOS | `/etc/tela/telafs.yaml` |
| Windows | `%ProgramData%\Tela\telafs.yaml` |

### telafs commands

```bash
telafs                              # start on default port
telafs -port <port>                 # start on custom port
telafs version                      # print version
telafs service install [-port <n>]  # install as OS service
telafs service start                # start service
telafs service stop                 # stop service
telafs service restart              # restart service
telafs service status               # show service status
telafs service uninstall            # remove service
```

---

## 9. Complete standalone walkthrough
...
# Start WebDAV server
telafs                              # default port 18080
telafs -port <port>                 # custom port

# Version
telafs version

# OS service management
telafs service install [-port <n>]
telafs service start | stop | restart | status | uninstall
```

...
type Hub struct {
	...
	// HubID is the hub's stable v4 UUID, as generated by telahubd on
	// first start and exposed on /.well-known/tela. Portals learn it
	// via the registration body (context 1) or by calling the hub's
	// /.well-known/tela (context 2). Immutable once stored.
	// See DESIGN-identity.md and DESIGN-portal.md section 3.2.
	HubID string
...
type HubVisibility struct {
	// ID is the hub's stable hubId UUID (DESIGN-portal.md section 3.1).
...
type Store interface {
	...
	// PortalID returns the portal's stable v4 UUID. Generated once on
	// the portal's first start and persisted in the store. Exposed on
	// /.well-known/tela as `portalId` (DESIGN-portal.md section 2 and
	// DESIGN-identity.md).
	PortalID() string

...
The browser opens `localhost:3000` and tries to call the API. The API is on a different origin (`localhost:4000`), so the browser refuses (Cross-Origin Resource Sharing (CORS)), or the UI has to be configured with the API URL, or there has to be an extra proxy layer somewhere.

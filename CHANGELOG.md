...
### Fixed
- Loopback listener on Windows routed connections to local services (sshd, Remote Desktop) instead of the tela tunnel even when tela held a more-specific socket binding. Root cause: the Remote Desktop kernel driver (`rdpwsx.dll`) intercepts port 3389 connections below Winsock and does not appear in any socket table; Windows OpenSSH binds `:::22` (IPv6 dual-stack) which is invisible to IPv4 socket enumeration. Detection is not possible for either case. Fix: on Windows, tela now always binds `remote + 10000` as the local port (e.g. SSH appears on port 10022, RDP on port 13389). TelaVisor displays the actual bound port from the `service_bound` event. On Linux and macOS the behavior is unchanged.

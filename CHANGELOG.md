...
### Added
- **CI coverage gate for security-critical packages.** CI now enforces a per-package statement-coverage floor for `internal/hub`, `internal/wsbind`, `internal/certpin`, `internal/credstore`, and `internal/client` so the security-critical paths cannot silently erode. A new CI-only tool, `tools/covcheck`, parses the coverage profile from `go test -covermode=atomic -coverprofile=coverage.out ./...` and fails the build (`ci.yml`) when any of those packages drops below the floor recorded in `tools/covcheck/floors.yaml`. Floors are set two points below today's measured coverage (`floor = max(1, floor(measured) - 2)`) and ratchet up only alongside real coverage gains. A package absent from the profile is treated as 0% coverage, so deleting a package's tests fails loudly. `internal/client`'s floor of 1 is an anti-deletion backstop pending card tela-71, which makes its security paths properly gateable. Closes #13.
...
### Fixed
- **Hub-to-portal registration now sends `hubId` in the POST body.** `registerWithPortal` omitted the `hubId` field that DESIGN-portal.md section 3.2 requires in context-1 (hub-bootstrap) registrations, even though the hub already emits its stable `hubId` on `/.well-known/tela` and `/api/status`. The outbound POST payload now includes `hubId` when the hub has one, so a conformant portal no longer has to fall back to a well-known lookup on the registering hub to learn it. Closes #10.
- **`tela admin access grant ... -services` no longer refuses every hub.** The client-side capability guard for service-scoped grants probed the wrong scheme: `fetchHubCapabilities` built its `/.well-known/tela` request straight from the `ws://`/`wss://` hub URL without routing through `wsToHTTP` first, so Go's default HTTP transport rejected every probe with "unsupported protocol scheme". The result was that `-services` grants were refused unconditionally, including against fully compliant v0.15.0+ hubs that genuinely advertise `per-service-access-control` (the guard failed closed, so it broadened no access, but it made the feature unusable). `fetchHubCapabilities` now converts the scheme via `wsToHTTP` before dialing, matching every other admin HTTP path. The refusal logic is extracted into a unit-testable `checkServicesCapability` helper, and a new `internal/client/capabilities_test.go` covers the capability probe, the `hubHasCapability` predicate, and the guard, including a regression test that drives the probe with a `ws://` URL. Closes #27.

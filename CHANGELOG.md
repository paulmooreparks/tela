...
- `telahubd` self-hosts release channels in-process under `/channels/`. A new `channels:` config block (enabled, data, publicURL) mounts `GET /channels/{name}.json` and `GET /channels/files/{binary}`. Accompanied by the `telahubd channels publish` subcommand that scans `channels.data/files/`, hashes the binaries, and writes `channels.data/{name}.json`.
- Remote publishing via admin API: `PUT /api/admin/channels/files/{name}` uploads binaries and `POST /api/admin/channels/publish` hashes them and writes the manifest. Lets a build pipeline publish to a self-hosted channel server over HTTPS with no SSH or file-share mount. Owner/admin auth required, 500 MiB per-file cap.

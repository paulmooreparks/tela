...
### Added
- TelaVisor has a new **Updates** tab in Clients mode (between Files and Client Settings) that replaces the Installed Tools card buried at the bottom of Client Settings. The tab surfaces the local-binary lifecycle (TelaVisor, tela CLI, any local telad/telahubd) with a full-width versions table, a *Check Now* button at the top, and a channel selector that mirrors the one in Application Settings so a single switch reconfigures both TelaVisor and the tela CLI. Channel Sources (custom channel URLs that feed every channel dropdown in the app) stays in Client Settings and the Updates tab links out to it. The topbar warning indicator continues to open its own update modal; indicator and tab share state so installing from either surface clears the other.
...
- Per-row services disclosure in the Access matrix: a compact "N of M services" button with a rotating chevron that toggles a chip cloud below. Small filters (three or fewer chips) auto-expand so the common case reads without interaction. The per-row **Edit...** button opens a modal with "Allow all services" / "Restrict to specific services" radio buttons; the restricted mode renders a scrollable checkbox list built from the machine's advertised services, or a free-text input for the wildcard `*` machine.
